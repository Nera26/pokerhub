...
describe('WalletService transactions', () => {
  ...
  beforeAll(async () => {
    ...
    service = new WalletService(accountRepo, journalRepo);
    await accountRepo.save([
      { id: '11111111-1111-1111-1111-111111111111', name: 'user', balance: 1000 },
  ...
  it('reserves and commits funds with rake and idempotency', async () => {
    ...
    const user = accounts.find((a) => a.id === '11111111-1111-1111-1111-111111111111');
  ...
  it('rolls back reservation', async () => {
    ...
    const reserve = await dataSource.getRepository(Account).findOneBy({ name: 'reserve' });
    expect(user?.balance).toBe(900);
    expect(reserve?.balance).toBe(0);
  });
});
