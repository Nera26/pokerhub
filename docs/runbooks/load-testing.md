...
1. Download the `k6-100k-chaos` artifact from CI (generated by `load/k6-table-actions.js` with `CHAOS_MODE=1`) and extract it under
   `load/metrics/<run-id>`.
...
2. Execute the k6 scenario:
   ```bash
   k6 run load/k6-table-actions.js \
     --summary-export=ws-ack-summary.json
   ```
3. Metrics are inserted into ClickHouse (`ws_ack_latency` table) and visualised via `load/grafana-ws-ack.json`.
