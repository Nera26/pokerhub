...
# start toxiproxy for packet loss/latency
"$SCRIPT_DIR/toxiproxy.sh"
...
if [[ -n "$HAND_HISTORY_FILE" ]]; then
  HAND_HISTORY_FILE="$HAND_HISTORY_FILE" SOCKETS="$SOCKETS" \
  ...
  SOCKETS="$SOCKETS" k6 run "$SCRIPT_DIR/k6-100k-sockets-replay.js" \
    --summary-export="$METRICS_DIR/k6-summary.json" \
    --out json="$METRICS_DIR/k6-metrics.json"
fi
