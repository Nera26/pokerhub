...
# inject fixed latency
toxiproxy-cli toxic add $PROXY_NAME -t latency -n latency -a latency=${LATENCY}
...
echo "Proxy $PROXY_NAME listening on :$PROXY_PORT → $UPSTREAM with ${LOSS} loss and ${LATENCY}ms latency"
