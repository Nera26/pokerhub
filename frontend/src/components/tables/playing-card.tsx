...
const suitPaths: Record<Suit, ReactNode> = {
  ...
  diamonds: <path d="M12 2l7 10-7 10-7-10 7-10z" />,
  clubs: (
...
function PlayingCard({
  ...
  const SuitIcon = () => (
    <svg viewBox="0 0 24 24" className={`${color} ${sz.centerSuit}`}>
      {suitPaths[suit]}
    </svg>
  );
  ...
  return (
    ...
    >
      ...
      />
      <div
        className={`absolute top-1 left-1 flex flex-col items-center leading-none ${color}`}
      >
        <span className={sz.rank}>{rank}</span>
        <svg viewBox="0 0 24 24" className={`${color} ${sz.cornerSuit}`}>
          {suitPaths[suit]}
        </svg>
      </div>
      <SuitIcon />
      <div
        className={`absolute bottom-1 right-1 rotate-180 flex flex-col items-center leading-none ${color}`}
      >
        <span className={sz.rank}>{rank}</span>
        <svg viewBox="0 0 24 24" className={`${color} ${sz.cornerSuit}`}>
          {suitPaths[suit]}
